...
![decision-tree-classes](./images/decision-tree-classes.PNG)
...
![decision-tree-classes-split](./images/decision-tree-classes-split.PNG)
...
![feature-space](./images/feature-space.PNG)
...
![least-squares-classification](./images/least-squares-classification.PNG)
...
![linear-classification-loss-functions](./images/linear-classification-loss-functions.PNG)
...
![stochastic-gradient-descent](./images/stochastic-gradient-descent.PNG)
...
![sgd-step-size](./images/sgd-step-size.PNG)
...
* For classification, the objective is to predict the correct class for $\textbf{x}_i$. An NN for classification has an output for each class
   ...
   * This requires true labels to be converted to one-hot encoded vectors:
      * E.g. for 3 classes, true label $\textbf{y}_i = [1 \space 0 \space 0]$ and predicted label $\hat{\textbf{y}}_i = [0.8186 \space 0.0907 \space 0.0907]$
...
![sgd-momentum](./images/sgd-momentum.PNG)
...
* Corrupts the input by using a probabilistic process before feeding it to the network:
   * $p(\v{x}_{ij}=0|x_{ij})=q$
   * $p(\v{x}_{ij}=x_{ij}|x_{ij})=1-q$
...
* Independent errors: probability that $k$ out of $N$ classifiers (with independent error rate $\epsilon$) are wrong:
   * $p(no. \space wrong \space classifiers = k) = \begin{pmatrix}N \\ k\end{pmatrix}\epsilon^k(1-\epsilon)^{n-k}$
...
![bagging-example](./images/bagging-example.PNG)
* $\hat{y}_i$ is independent of $\tilde{y}_i$ 
   * This gives us a distrbution over the loss for $\tilde{\textbf{x}}_i$ with $\mathbb{E}[\mathcal{L}(\hat{y}_i,\tilde{y}_i)|\tilde{\textbf{x}}_i]$
...
* Bootstrapping selects approximately 63% of the original samples for large $n$:
   * $p(\textbf{x}_i \space selected \space in \space at \space least \space one \space of \space n \space trials) = 1 - p(\textbf{x}_i \space not \space selected \space in \space any \space of \space n \space trials)$
   * $=1-p(\textbf{x}_i \space not \space selected \space in \space a \space trial)^n$ (sincre trials are independent)
   * $=1-(1-\frac{1}{n})^n$
   * $=1-\frac{1}{e}$
   * $\approx 0.63$
...
* Recall our linear classifier for two samples used to explain decision boundaries:
   * $\hat{y}_i = w_1x_{i1}+w_2x_{i2}$ with $\textbf{w} = [1 \space -2]$ and no bias
...
![linear-classifier-bias](./images/linear-classifier-bias.PNG)
...
* Ensemble can overfit if trees are too deep or there are too many trees
   * To restrict the depth of each tree, XGBoost adds a penalty $f(w_1, w_2, ..., w_L) = \sum_{i=1}^n (w_l^i-r_i)^2 + \lambda ||\textbf{w}||_0$ where $\textbf{w} = [w_1 \space w_2 \space ... \space w_L]$
